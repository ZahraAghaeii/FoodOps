...
- 📜 **Detailed Status Change Logging:** Automated tracking and logging of every order status transition, storing who made the change, the old status, the new status, and the exact timestamp.
...
### 4. OrderLog Schema (`OrderLog.js`)

- `orderId`: Reference to `Order` Object ID (Required)
- `oldStatus`: Previous status of the order (String)
- `newStatus`: Updated status of the order (String, Required)
- `changedAt`: Timestamp of the status change (Date, Default: `Date.now`)
- `changedBy`: Reference to `User` ID who performed the status change
...
| `GET` | `/:id/logs` | Fetch audit status logs for a specific order |
...
### 4. Detailed Status Change Audit Logging

Every status transition (e.g., Pending -> Preparing -> Ready -> Delivered) creates an immutable audit record in the `OrderLog` collection, ensuring traceability of operational workflows.

### 5. Data Protection & Hashing
